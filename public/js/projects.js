...
const gitGetReposUser = async () => {
  const repos = await fetch(
    `http://api.github.com/users/${userParams}/repos`
  ).then((res) => res.json());

  const language = await fetch(`http://localhost:3000/languages`).then((res) =>
    res.json()
  );
  ...
  repos.map(async (repo) => {
    ...
    template += `
        <div onclick="redirect('${info.name}','${info.login}')" class="project">

          <div class="titleCard">
            <h2>${
              info.name.length > 8 ? info.name.slice(0, 9) + "..." : info.name
            }</h2>
            <p class="visibility">${info.visibility}</p>
          </div>

          <div class="textCard">
            <p>${
              info.description != null && info.description?.length > 69
                ? info.description.slice(0, 69) + "..."
...
const redirect = (repo, user) => {
  window.location.href = `/public/projectShow.html?repo=${repo}&user=${user}`;
};
